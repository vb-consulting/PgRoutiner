...
namespace PgRoutiner
{
    partial class Builder
    {
        private static void BuildDump(string dumpFile, Func<string> contentFunc, Action<string> successFunc)
        {
            ...
            var file = string.Format(Path.GetFullPath(Path.Combine(Program.CurrentDir, dumpFile)), Settings.Value.Connection);
            var shortName = string.Format(dumpFile, Settings.Value.Connection);
