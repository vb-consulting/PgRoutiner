...
{
    public partial class CrudReadByCode : CrudCodeBase
    {
        ...
        {
            var name = $"Read{Name.ToUpperCamelCase()}By{string.Join("And", Pk.Select(p => p.Name.ToUpperCamelCase()).ToArray())}Async";
            var actualReturns = $"ValueTask<{this.Model}>";
            Class.AppendLine();
            BuildAsyncMethodCommentHeader();
            Class.AppendLine($"{I2}public static async {actualReturns} {name}(this NpgsqlConnection connection, {string.Join(", ", this.Pk.Select(p => $"{p.Type} {p.Name}").ToArray())})");
            Class.AppendLine($"{I2}{{");
        ...
        protected override void BuildExpressionBodySyncMethod()
        {
            ...
            Class.AppendLine();
            BuildSyncMethodCommentHeader();
            Class.AppendLine($"{I2}public static {actualReturns} {name}(this NpgsqlConnection connection, {string.Join(", ", this.Pk.Select(p => $"{p.Type} {p.Name}").ToArray())}) => connection");
            if (!settings.CrudNoPrepare)
            {
            ...
            {
                Class.AppendLine(", ");
                Class.Append(string.Join($",{NL}", Pk.Select(p => $"{I4}(\"{p.PgName}\", {p.Name}, {p.DbType})")));
            }
            Class.AppendLine($")");
        ...
        protected override void BuildExpressionBodyAsyncMethod()
        {
            ...
            Class.AppendLine();
            BuildAsyncMethodCommentHeader();
            Class.AppendLine($"{I2}public static async {actualReturns} {name}(this NpgsqlConnection connection, {string.Join(", ", this.Pk.Select(p => $"{p.Type} {p.Name}").ToArray())}) => await connection");
            if (!settings.CrudNoPrepare)
            {
                Class.AppendLine($"{I3}.Prepared()");
            }
